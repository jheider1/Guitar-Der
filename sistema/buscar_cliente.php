...
<head>
	<meta charset="UTF-8">
	<title>Buscar cliente - Clothes For Me</title>
...
<body>
	...
	<section id="container">
		...
		<a class="btn_inac" href="clientes_eliminados">Clientes inactivos</a>
		<a class="btn_desc" href="reporte_clientes">Descargar reporte</a>
		...
		<table>
			...
			$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM clientes WHERE (idCliente LIKE '%$busqueda%' OR cedula LIKE '%$busqueda%' OR nombres LIKE '%$busqueda%' OR apellidos LIKE '%$busqueda%' OR correo LIKE '%$busqueda%' OR telefono LIKE '%$busqueda%' OR celular LIKE '%$busqueda%' OR direccion LIKE '%$busqueda%' OR genero LIKE '%$busqueda%') AND status = 1");
			...
			$total_paginas = ceil($total_registro / $por_pagina);
				...
				$query = mysqli_query($conection, "SELECT * FROM clientes WHERE
																...
																AND 
																status = 1 ORDER BY idCliente LIMIT $desde,$por_pagina");
				mysqli_close($conection);
			...
			<tbody>
             ...
             ?>
					<td data-titulo="acción">
						<a class="link_edit" href="editar_cliente?id=<?php echo $data['idCliente']; ?>">Editar</a>
						
						 <span>|</span>

						<a class="link_delete" href="eliminar_cliente?id=<?php echo $data['idCliente']; ?>">Eliminar</a>
					</td>
				<?php } ?>
				</tr>
			</tbody>
