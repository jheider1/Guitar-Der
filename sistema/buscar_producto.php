...
<body>
	...
	<section id="container">
		...
		<table>
			...
			$sql_registe = mysqli_query($conection,"SELECT COUNT(*) as total_registro FROM productos WHERE (idProductos LIKE '%$busqueda%' OR nombre LIKE '%$busqueda%' OR descripcion LIKE '%$busqueda%' OR precio LIKE '%$busqueda%' OR existencia LIKE '%$busqueda%' OR categoria LIKE '%$busqueda%' OR tipo LIKE '%$busqueda%' OR modelo LIKE '%$busqueda%' OR imagen) AND status = 1");
			...
			$total_paginas = ceil($total_registro / $por_pagina);
				...
				$query = mysqli_query($conection, "SELECT * FROM productos WHERE (idProductos LIKE '%$busqueda%' OR 
																						   ...
																						   AND 
																						   status = 1 ORDER BY idProductos DESC LIMIT $desde,$por_pagina");
				mysqli_close($conection);
			...
			<tbody>
				<tr>
					...
					<?php $foto = 'img/uploads/'.$data['imagen']; ?>
	          		...
	          		<?php if ($data['idProductos'] != 1) {
						 ?>
					<td data-titulo="acción">
						<a class="link_edit" href="editar_producto.php?id=<?php echo $data['idProductos']; ?>">Editar</a>
						
						 <span>|</span>
						<a class="link_delete" href="eliminar_producto.php?id=<?php echo $data['idProductos']; ?>">Eliminar</a>
					</td>
				<?php } ?>
				</tr>
			</tbody>
